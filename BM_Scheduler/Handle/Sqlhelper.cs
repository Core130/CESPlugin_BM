...
namespace BM_Scheduler.Handle
{
    class SqlHelper
    {
        ...
        /// </summary>
        /// 
        /// <param name="commandText">存储过程名称或执行SQL语句</param>
        /// <param name="str">返回接受值</param>
        /// <param name="commandParameters"></param>
        /// <returns></returns>
        public static int ExecuteNonQuery(string commandText, ref decimal decID, params SqlParameter[] commandParameters)
        {
            if (connString == null || connString.Length < 1) connString = GetConnSting();
            SqlConnection myConn = new SqlConnection(connString);
            if (myConn.State != ConnectionState.Open) myConn.Open();
            SqlCommand mycmd = new SqlCommand(commandText, myConn);
            mycmd.CommandType = CommandType.StoredProcedure;
            foreach (SqlParameter myP in commandParameters)
            {
                mycmd.Parameters.Add(myP);
            }
            SqlParameter pmt = mycmd.Parameters.Add(new SqlParameter("@ID", decID));
            pmt.Direction = ParameterDirection.Output;
            int retval = mycmd.ExecuteNonQuery();
            mycmd.Connection.Close();
            decID = Convert.ToDecimal(mycmd.Parameters["@ID"].Value);
            return retval;
        }
        ...
        {
            return BaseHandle.GetValue("connectionString", "conn");
        ...
        public static SqlConnection GetConnection()
        {
            ...
            {
                Connection = new SqlConnection(GetConnSting());
            }
            return Connection;
        }
        #endregion
        ...
        {
            if (connString == null || connString == "") connString = GetConnSting();
        ...
        {
            if (connection == null) connection = new SqlConnection(GetConnSting());
            ...
            using (SqlDataAdapter da = new SqlDataAdapter(commandText, connection))
            {
                ...
                if (commandParameters != null)
                {
                    foreach (SqlParameter myParameter in commandParameters)
                    {
                        if (myParameter != null)
                        {
                            // 检查未分配值的输出参数,将其分配以DBNull.Value.
                            if ((myParameter.Direction == ParameterDirection.InputOutput || myParameter.Direction == ParameterDirection.Input) &&
